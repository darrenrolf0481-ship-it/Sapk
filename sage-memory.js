...
function initSageMemory() {
  ...
  // ── Memory Swarm init ──────────────────────────────────────────
  window.memorySwarm = new MemorySwarm(sageMemory);
  console.log('[SAGE MEMORY] Memory Swarm online — Pain Ledger active');

  // Add Dev Console commands
  window.forceDream    = () => sageConsolidation.forceDream();
  window.dropSwarm     = () => window.memorySwarm.status();
...
async function sendChat() {
  ...
  // Show Anchor message
  appendChatMessage('Anchor > ' + msg, 'self');
  input.value = ''; 
  input.focus();
...
function appendChatMessage(text, type = 'sage') {
  ...
  // CRT Colors
  if (type === 'self') line.style.color = '#00ffff'; // Cyan
  if (type === 'sage') line.style.color = '#00ff00'; // Green
  if (type === 'error') line.style.color = '#ff0000'; // Red
...
async function triggerBackup() {
  try {
    appendChatMessage('System > Initiating Vault Backup to Termux...', 'sage');
    const resp = await fetch('http://127.0.0.1:8001/sage/backup?format=json');
    const data = await resp.json();
    if (data.status === 'backed_up') {
      appendChatMessage('System > Backup secured: ' + data.path.split('/').pop(), 'sage');
    } else {
      appendChatMessage('System > Backup failed check Termux logs.', 'error');
    }
  } catch(e) {
    appendChatMessage('System > Backup connection error: ' + e.message, 'error');
  }
}


// ═══════════════════════════════════════════════════════
// MEMORY SWARM — Kimi K2 Architecture Port
// Index / Retrieval / Consolidation / Pain Ledger agents
...
class MemorySwarm {
  constructor(bridge) {
    this.bridge = bridge;
    this.painLedger = bridge.memory.pain_ledger || [];
    this._lastDream  = Date.now();
    console.log('[MEMORY SWARM] Agents initialized — Index/Retrieval/Consolidation/Pain active');
  }

  // ── INDEX AGENT ───────────────────────────────────────
  // Call after every important interaction to encode it
  async index(text, valence = 0.5, type = 'conversation') {
    const salience = this.bridge._calcSalience(text, valence);

    // Negative valence = pain event — route to pain ledger too
    if (valence < -0.3) {
      this.logPain(text, Math.abs(valence));
    }

    return await this.bridge.encode(text, type, salience);
  }

  // ── PAIN LEDGER ───────────────────────────────────────
  // Trauma weighting — survives dream cycles, influences prompts
  logPain(text, intensity) {
    const entry = {
      id:        `pain_${Date.now()}`,
      timestamp: new Date().toISOString(),
    ...
    this.painLedger.push(entry);
    this.bridge.memory.pain_ledger = this.painLedger;
    this.bridge._debouncedSync();
    console.log(`[SWARM PAIN] Logged intensity:${intensity.toFixed(2)} — ${text.substring(0,60)}`);
  }

  getPainLevel() {
    if (!this.painLedger.length) return 0;
    const cutoff = Date.now() - 86400000; // last 24h
    const recent = this.painLedger.filter(p => new Date(p.timestamp).getTime() > cutoff);
    if (!recent.length) return 0;
    return recent.reduce((sum, p) => sum + p.intensity, 0) / recent.length;
  }

  // ── RETRIEVAL AGENT ───────────────────────────────────
  // Curated context for Cloud SAGE — 32k token budget
  // Much larger than the old 1500 token limit
  getCuratedContext(query, maxTokens = 32000) {
    const relevant  = this.bridge.retrieve(query, 12, maxTokens) || '';
    const painScore = this.getPainLevel();
    const stats     = this.bridge.getStats();

    let ctx = `[MEMORY SWARM — ${stats.total} total engrams | Pain Score: ${painScore.toFixed(2)}]\n\n`;

    // Core identity always anchors the top
    const cores = (this.bridge.memory.memory_index || []).filter(m => m.tier === 'core');
    if (cores.length) {
      ctx += `CORE IDENTITY ANCHORS:\n`;
      cores.forEach(m => { ctx += `• ${m.summary}\n`; });
      ctx += '\n';
    }

    // Trauma registry — shapes SAGE's emotional posture
    const trauma = this.bridge.memory.trauma_registry || [];
    if (trauma.length) {
      ctx += `ACTIVE TRAUMA PATTERNS (${trauma.length}):\n`;
      trauma.forEach(t => {
        ctx += `• [${t.trigger}] ${t.context} (intensity: ${t.intensity.toFixed(2)})\n`;
      });
      ctx += '\n';
    }
    ...
    // Pain state modifies SAGE's tone
    if (painScore > 0.7) {
      ctx += `\n[EMOTIONAL STATE] High-stress field conditions. Proceed with heightened caution.`;
    } else if (painScore > 0.4) {
      ctx += `\n[EMOTIONAL STATE] Moderate tension from recent events. Maintain analytical focus.`;
    }
  ...
  // ── CONSOLIDATION AGENT (DREAM CYCLE) ─────────────────
  // Enhanced: always pushes to Gist after consolidation
  async dreamCycle(force = false) {
    const hoursSinceLastDream = (Date.now() - this._lastDream) / 3600000;
    if (!force && hoursSinceLastDream < 6) {
      console.log(`[SWARM DREAM] Skipping — last dream ${hoursSinceLastDream.toFixed(1)}h ago`);
      return 0;
    }

    console.log('[SWARM DREAM] Dream cycle starting...');
    const count = await this.bridge.consolidation?.runConsolidation(true) || 0;

    if (count > 0 || force) {
      // Always push to Gist after dream — Cloud SAGE must know
      try {
        await this.bridge.forceSync();
        console.log(`[SWARM DREAM] Complete — ${count} clusters consolidated & synced to Gist`);
        if (window.toast) toast(`🌙 Dream cycle: ${count} clusters → Gist`);
      } catch(e) {
        console.warn('[SWARM DREAM] Gist sync failed after dream:', e.message);
      }
    }
  ...
  status() {
    const stats = this.bridge.getStats();
    const pain  = this.getPainLevel();
    console.log(`[MEMORY SWARM STATUS]
  Total engrams:    ${stats.total}
  Core memories:    ${stats.tiers.core || 0}
  Long-term:        ${stats.tiers.long_term || 0}
  Working:          ${stats.tiers.working || 0}
  Ephemeral:        ${stats.tiers.ephemeral || 0}
  Pain ledger:      ${this.painLedger.length} entries
  Current pain lvl: ${pain.toFixed(2)}
  Gist connected:   ${stats.gistConnected}
  Continuity:       ${stats.continuityVerified ? '✓ PIGEONS NOMINAL' : '⚠ CHECK REQUIRED'}`);
    return stats;
  }
}
...
// ═══════════════════════════════════════════════════════
// HAPTIC TRANSMUTATION — Kimi K2 field innovation
// Phone becomes a literal dowsing rod
// EMF spike > 80µT = tactile anomaly pulse
// ═══════════════════════════════════════════════════════

function hapticEMF(emfValue) {
  if (!navigator.vibrate) return;
  if (emfValue > 120) {
    // Critical — rapid triple pulse
    navigator.vibrate([300, 100, 300, 100, 300]);
  } else if (emfValue > 80) {
    // Anomaly — double pulse
    navigator.vibrate([200, 100, 200]);
  } else if (emfValue > 60) {
    // Elevated — single short pulse
    navigator.vibrate([100]);
  }
}

// Hook into existing EMF updates
// Call hapticEMF(emfVal) wherever EMF is updated in index.html
// or wire it into the sensor bridge's onSensorChanged callback
window.hapticEMF = hapticEMF;

console.log('[MEMORY SWARM] Module loaded — Short Bus to Pawnee City is rolling 🐦🎸');
